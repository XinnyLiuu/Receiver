...
export class ModalComponent implements OnInit {
	...
	ngOnInit() {
		/**
		 * Prepare an array of languages for the user to select from
		 * 
		 * https://pkgstore.datahub.io/core/language-codes/language-codes-3b2_json/data/529809cd9e4c8829ec80dc4d2b2997e9/language-codes-3b2_json.json
		 */
		this.languages = [
			{
				"English": "Afar",
		...
		// Set default error flag to false
		this.error = false;
		this.errorMessage = "An error has occured!"
	}

	/**
	 * Close the modal and determine what data to return to the page that opened this modal
	 * 
	 * @param data 
	 */
	dismiss(data: any) {
		...
		if (data === "Get Data") {
			// Get DOM values and validate
			const lang: HTMLSelectElement = document.querySelector("#select");
			const text: HTMLInputElement = document.querySelector("#input");

			if (lang.value !== undefined && text.value !== "") {
				return this.modalController.dismiss({
					"dismissed": true,
					lang: lang.value,
					text: text.value
				...
				this.error = true;
				this.errorMessage = "Please check all fields have been filled!";
			}
		}
	}
}
